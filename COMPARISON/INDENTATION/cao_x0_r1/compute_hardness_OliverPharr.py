...
apex_angle_deg = 74.68      # total included cone angle φ (nuestra punta: r=40, h=52.5 A)
...
# OJO: no usar la cola final (post-descarga) para decidir el signo -- ahi la
# fuerza es puro ruido (~1e-12) con signo esencialmente aleatorio, y una
# mediana tomada ahi puede invertir toda la curva por casualidad. Usamos el
# signo del punto de mayor magnitud (bien dentro de la señal real) en su lugar.
i_absmax = np.argmax(np.abs(F0))
F = -F0 if F0[i_absmax] < 0 else F0
