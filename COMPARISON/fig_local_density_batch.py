"""
fig_local_density_batch.py
CN_O (O coordination number) alrededor de Na y K — r_cut uniforme.
r_cut = 3.77 Å (primer mínimo K–O, sistema PMMCS) — mismo para Na y K.
Genera fig_cn_XX{20,40,60,80,100}.pdf — 1 fila, 2 paneles: 10CaO | Ca-free.
...
RCUT_NA = 3.21   # primer mínimo Na–O g(r), sistema PMMCS
RCUT_K  = 3.77   # primer mínimo K–O  g(r), sistema PMMCS
...
def cn_around(pos, types, box, center_type):
...
for XX, (xna_cao, xka_cao, xt_cao,
    ...
    print(f"\n── XX={XX}%  CaO(Na x={xna_cao}, K x={xka_cao}, xt={xt_cao}) "
          f"  Ca-free(Na x={xna_noca}, K x={xka_noca}, xt={xt_noca}) ──")

    cao_cn_na  = load_am(CAO_AM_DIR,      xna_cao,  TYPE_NA)
    cao_cn_ka  = load_am(CAO_AM_DIR,      xka_cao,  TYPE_K)
    cao_cn_ki  = load_iex1(CAO_IEX1_DIR,  xt_cao,   TYPE_K)
    noca_cn_na = load_am(NOCA_AM_DIR,     xna_noca, TYPE_NA)
    noca_cn_ka = load_am(NOCA_AM_DIR,     xka_noca, TYPE_K)
    noca_cn_ki = load_iex1(NOCA_IEX1_DIR, xt_noca,  TYPE_K)
