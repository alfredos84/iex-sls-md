...
for ((i=0; i<${#x_values[@]}; i++)); do
  x=${x_values[i]}
  mpirun -np 4 lmp -in input_lammps_x.in -var val_x ${x} -var T_end ${t_final}
  echo "  - input_lammps_x.in simulation: x=${x}, t_final=${t_final}"
  mpirun -np 4 lmp -in input_lammps_y.in -var val_x ${x} -var T_end ${t_final}
  echo "  - input_lammps_y.in simulation: x=${x}, t_final=${t_final}"
  mpirun -np 4 lmp -in input_lammps_z.in -var val_x ${x} -var T_end ${t_final}
