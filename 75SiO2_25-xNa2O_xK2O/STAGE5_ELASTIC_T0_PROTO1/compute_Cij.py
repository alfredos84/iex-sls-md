"""
compute_Cij.py — STAGE5_ELASTIC_T0_PROTO1
...
Usage:
  python compute_Cij.py --xt 6 --r 2 --results results/
"""
...
def build_Cij(xt, r, results_dir):
    results = Path(results_dir)
    C = np.zeros((6, 6))

    for d in range(1, 7):
        neg_file = results / f"elastic_T0_stress_xt{xt}_r{r}_dir{d}_sign-1.txt"
        pos_file = results / f"elastic_T0_stress_xt{xt}_r{r}_dir{d}_sign1.txt"

        if not neg_file.exists() or not pos_file.exists():
            print(f"  WARNING: missing stress files for dir={d}, xt={xt}, r={r}",
                  file=sys.stderr)
            continue
...
def main():
    ...
    parser.add_argument('--xt',      type=int, required=True, help='x_target (1,3,6,9,12,15)')
    parser.add_argument('--r',       type=int, required=True, help='replica (1,2,3)')
    parser.add_argument('--results', type=str, default='results')
    args = parser.parse_args()

    C = build_Cij(args.xt, args.r, args.results)
    KV, GV, K, G, E, nu = voigt_averages(C)

    print(f"\n=== Elastic tensor (GPa) — PMMCS rc8p0 IEX-Proto1, xt={args.xt}, r={args.r} ===")
    ...
    out = Path(args.results) / f"elastic_T0_Cij_PMMCS_rc8p0_IEX1_xt{args.xt}_r{args.r}.txt"
