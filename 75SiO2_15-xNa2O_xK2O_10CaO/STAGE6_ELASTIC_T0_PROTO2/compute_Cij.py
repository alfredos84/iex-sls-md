"""
compute_Cij.py — STAGE6_ELASTIC_T0_PROTO2
...
Usage:
  python compute_Cij.py --xp 6 --r 2 --results results/
"""
...
def build_Cij(xp, r, results_dir):
    results = Path(results_dir)
    C = np.zeros((6, 6))

    for d in range(1, 7):
        neg_file = results / f"elastic_T0_stress_xp{xp}_r{r}_dir{d}_sign-1.txt"
        pos_file = results / f"elastic_T0_stress_xp{xp}_r{r}_dir{d}_sign1.txt"

        if not neg_file.exists() or not pos_file.exists():
            print(f"  WARNING: missing stress files for dir={d}, xp={xp}, r={r}",
                  file=sys.stderr)
            continue
...
def main():
    ...
    parser.add_argument('--xp',     type=int, required=True, help='x_parent (0,1,3,6,9,12)')
    parser.add_argument('--r',      type=int, required=True, help='replica (1,2,3)')
    parser.add_argument('--results', type=str, default='results')
    args = parser.parse_args()

    C = build_Cij(args.xp, args.r, args.results)
    KV, GV, K, G, E, nu = voigt_averages(C)

    print(f"\n=== Elastic tensor (GPa) — PMMCS rc8p0 IEX-Proto2, xp={args.xp}, r={args.r} ===")
    ...
    out = Path(args.results) / f"elastic_T0_Cij_PMMCS_rc8p0_IEX2_xp{args.xp}_r{args.r}.txt"
